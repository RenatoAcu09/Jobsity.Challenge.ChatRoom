...
namespace Jobsity.Challenge.ChatRoom.UnitTests.UseCases.Services
{
    public class AddToRoomUseCaseTests
    {
        ...
        public async Task AddAsync_ShouldReturnRoom_WhenIsNewUser()
        {
            ...
            _chatRoomRepository
                ...
                .ReturnsAsync(new Domain.Entities.ChatRoom());
            _mapper
                .Setup(m => m.Map<ChatRoomDto>(It.IsAny<Domain.Entities.ChatRoom>()))
            ...
            _chatRoomRepository.Verify(
                                       ...
                                       Times.Once());
            _mapper.Verify(
                           m => m.Map<ChatRoomDto>(It.IsAny<Domain.Entities.ChatRoom>()),
                           Times.Once());
        }
    }
}